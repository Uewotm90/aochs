...
transformStep2 :: Integer -> [Integer]
transformStep2 n
  | n == 0 = [2024]
  | evenDigit n = [20, 24]
  | otherwise = transform $ n * 2024

blinkStep2 xs = concatMap transformStep2 xs

blinks' :: Int -> [Integer] -> [Integer]
blinks' n xs
  | n == 0 = xs
  | n == 1 = blink xs
  | n > 1 = blinks' (n - 2) $ blinkStep2 xs

secondSol = do
  input <- readFile "./input/11.txt"
  return $ length $ blinks' 75 $ fst $ head $ parse p input